...
struct Transform_WellKnownOp_Fixture_Tag;
C2H_TEST("Transform works with integral types with well-known operations", "[transform][well_known]", integral_types)
{
  ...
  auto& build_cache    = get_cache<Transform_WellKnownOp_Fixture_Tag>();
  const auto& test_key = make_key<T>();
...
operation_t make_double_pair_op()
{
  return make_operation(
    "double_pair_op",
    "struct pair { short a; size_t b; };\n"
    "extern \"C\" __device__ void double_pair_op(void* x_ptr, void* out_ptr) {\n"
...
C2H_TEST("Transform works with custom types", "[transform]")
{
  ...
  operation_t op              = make_double_pair_op();
...
struct Transform_WellKnownOp_CustomType_Fixture_Tag;
C2H_TEST("Transform works with custom types with well-known operators", "[transform][well_known]")
{
  const std::size_t num_items = GENERATE(0, 42, take(4, random(1 << 12, 1 << 24)));

  // Important: do not elide op_state, it ensure lifetime of strings
  // referenced to from cccl_op_t
  operation_t op_state = make_double_pair_op();
  ...
  auto& build_cache    = get_cache<Transform_WellKnownOp_CustomType_Fixture_Tag>();
