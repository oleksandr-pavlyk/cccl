...
struct UniqueByKey_WellKnownOp_Fixture_Tag;
C2H_TEST("DeviceSelect::UniqueByKey works with well-known operations", "[unique_by_key][well_known]", key_types)
{
  ...
  auto& build_cache = get_cache<UniqueByKey_WellKnownOp_Fixture_Tag>();
  // key: (input_type, output_type, num_selected_type)
  const auto& test_key = make_key<key_t, item_t, int>();
...
operation_t make_pair_equal()
{
  return make_operation(
    "pair_equal",
    "struct key_pair { short a; size_t b; };\n"
    "extern \"C\" __device__ void pair_equal(const void* lhs_ptr, const void* rhs_ptr, void* out_ptr) {\n"
...
C2H_TEST("DeviceSelect::UniqueByKey works with custom types", "[device][select_unique_by_key]")
{
  ...
  operation_t op              = make_pair_equal();
...
struct UniqueByKey_WellKnownOp_CustomType_Fixture_Tag;
         ...
         "[device][select_unique_by_key][well_known]")
{
  ...
  // Important: do not elide op_state, it ensures lifetime of strings
  // referenced to by cccl_op_t
  operation_t op_state        = make_pair_equal();
  ...
  auto& build_cache    = get_cache<UniqueByKey_WellKnownOp_CustomType_Fixture_Tag>();
  const auto& test_key = make_key<key_pair, item_t, int>();
