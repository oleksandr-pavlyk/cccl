...
struct DeviceMergeSort_WellKnownOp_Fixture_Tag;
C2H_TEST("DeviceMergeSort::SortKeys works with well-known predicate", "[merge_sort][well_known]", key_types)
{
  ...
  auto& build_cache    = get_cache<DeviceMergeSort_WellKnownOp_Fixture_Tag>();
  const auto& test_key = make_key<key_t>();
...
using key_pair  = pair_t<short, size_t>;
using item_pair = pair_t<int, float>;

operation_t make_pair_less()
{
  return make_operation(
    "pair_less",
    "struct key_pair { short a; size_t b; };\n"
    "extern \"C\" __device__ void pair_less(const void* lhs_ptr, const void* rhs_ptr, void* out_ptr) {\n"
    "  const key_pair* lhs = static_cast<const key_pair*>(lhs_ptr);\n"
    "  const key_pair* rhs = static_cast<const key_pair*>(rhs_ptr);\n"
...
C2H_TEST("DeviceMergeSort:SortPairsCopy works with custom types", "[merge_sort]")
{
  ...
  std::sort(expected_keys.begin(), expected_keys.end());
  std::sort(expected_items.begin(), expected_items.end());
  REQUIRE(std::equal(expected_keys.begin(), expected_keys.end(), std::vector<key_pair>(output_keys_it).begin()));
  REQUIRE(std::equal(expected_items.begin(), expected_items.end(), std::vector<item_pair>(output_items_it).begin()));
}

struct DeviceMergeSort_WellKnownOp_SortPairsCopy_CustomType_Fixture_Tag;
C2H_TEST("DeviceMergeSort:SortPairsCopy works with custom types with well-known predicates", "[merge_sort][well_known]")
{
  ...
  // This temporary is mandatory to prevent deletion
  // of name/code data stored in operation_t,
  // which cccl_op_t only refer to by pointers
  operation_t op_state = make_pair_less();
  cccl_op_t op         = op_state;
  op.type              = cccl_op_kind_t::CCCL_LESS;

  auto& build_cache    = get_cache<DeviceMergeSort_WellKnownOp_SortPairsCopy_CustomType_Fixture_Tag>();
  const auto& test_key = make_key<key_pair, item_pair>();
