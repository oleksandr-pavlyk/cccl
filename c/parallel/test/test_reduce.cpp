...
using integral_types = c2h::type_list<int32_t, uint32_t, int64_t, uint64_t>;
struct Reduce_WellKnownOp_Fixture_Tag;
C2H_TEST("Reduce works with integral types with well-known operations", "[reduce][well_known]", integral_types)
{
  ...
  auto& build_cache    = get_cache<Reduce_WellKnownOp_Fixture_Tag>();
  const auto& test_key = make_key<T>();
...
operation_t make_pair_less()
{
  return make_operation(
    ...
    "extern \"C\" __device__ void pair_plus(void* lhs_ptr, void* rhs_ptr, void* out_ptr) {\n"
...
struct DeviceReduce_WellKnownOp_CustomType_Fixture_Tag;
C2H_TEST("Reduce works with custom types with well-known operations", "[reduce][well_known]")
{
  const std::size_t num_items = GENERATE(0, 42, take(4, random(1 << 12, 1 << 24)));

  // Important: rvalue op_state ensures life-time of strings of operator name and code
  // DO NOT elide op_state
  ...
  auto& build_cache    = get_cache<DeviceReduce_WellKnownOp_CustomType_Fixture_Tag>();
  const auto& test_key = make_key<pair>();
