...
using integral_types = c2h::type_list<int32_t, uint32_t, int64_t, uint64_t>;
struct Scan_WellKnownOp_Fixture_Tag;
C2H_TEST("Scan works with integral types with well-known operations", "[scan][well_known]", integral_types)
{
  ...
  auto& build_cache    = get_cache<Scan_WellKnownOp_Fixture_Tag>();
  const auto& test_key = make_key<T>();
...
operation_t make_pair_plus()
{
  return make_operation(
    ...
    "extern \"C\" __device__ void pair_plus(const void* lhs_ptr, const void* rhs_ptr, void* out_ptr) {\n"
    "  const pair* lhs = static_cast<const pair*>(lhs_ptr);\n"
    "  const pair* rhs = static_cast<const pair*>(rhs_ptr);\n"
...
struct Scan_WellKnownOp_CustomType_Fixture_Tag;
C2H_TEST("Scan works with custom types with well-known operations", "[scan][well_known]")
{
  const std::size_t num_items = GENERATE(0, 42, take(4, random(1 << 12, 1 << 24)));

  // Important: do not elide op_state, it ensure lifetime
  // of strings defining functions and it name
  operation_t op_state        = make_pair_plus();
  ...
  auto& build_cache    = get_cache<Scan_WellKnownOp_CustomType_Fixture_Tag>();
  const auto& test_key = make_key<pair>();
